...
import { API_FILE_UPLOAD, API_GET_PARTNER_PROFILE, API_GET_USER_PROFILE, API_UPDATE_PARTNER_PASSWORD, API_UPDATE_PARTNER_PROFILE, API_UPDATE_USER_PASSWORD, API_UPDATE_USER_PROFILE } from "@/services/api/endpoints";
...
export default function ProfileSetting() {
    ...
    const { isLoading, isError, error, data, isSuccess } = useQuery({
        queryKey: ['singlePartner'],
        queryFn: () => get(API_GET_PARTNER_PROFILE),
        // enabled: !!id
    });


    const partnerData = isSuccess ? data?.data : {};
    ...
    const updateMutation = useMutation(
        async (data) => await patch(`${API_UPDATE_PARTNER_PROFILE}`, data),
        {
            onSuccess: (res) => {
            ...
            onError: (err) => {
                if (err && err?.response) {
                    setProfileUpdateError(err?.response?.message);
                }
            }
        }
    );
    ...
    const passwordMutation = useMutation(
        async (data) => await patch(`${API_UPDATE_PARTNER_PASSWORD}`, data),
        {
            onSuccess: (res) => {
                showToast("success", "Updated Successfully")
    ...
    const getProfileImage = () => {
        ...
        if (image === null && partnerData?.profile_photo) {
            return (
                <img
                    className="upload-avater-img"
                    src={partnerData?.profile_photo}
                    alt=""
                />
            )
        }
